...
function validateSite(root){
  ...
  for(const f of files){
    ...
    if(!/^(index\.html|site\.css|\.nojekyll|evidence\/lesson-0[1-8]\.html|previews\/lesson-0[1-8]\.html|assets\/fonts\/(workshop-hand\.woff2|OFL\.txt|provenance\.json)|assets\/lesson-0[1-8]\/reader\/fig-0[1-6]-[a-z-]+\.png)$/.test(rel))throw new Error(`Not publishable: ${rel}`)
    if(f.endsWith('.png')){
  ...
  for(const required of ['index.html','assets/fonts/OFL.txt',...['01','02','03','04','05','06','07','08'].flatMap(id=>[`previews/lesson-${id}.html`,`evidence/lesson-${id}.html`])])if(!fs.existsSync(path.join(root,required)))throw new Error(`Missing ${required}`)
...
if(require.main===module){
  ...
  if(process.argv.includes('--repository')){
    ...
    for(const file of files)if(!/^(reader\/|README\.md$|\.gitattributes$|\.github\/workflows\/pages\.yml$|scripts\/check-reader\.cjs$|releases\/lesson-(01-v1\.0\.1|01-03-v1\.1\.0|01-05-v1\.2\.0|01-08-v1\.3\.0)\.md$)/.test(file))throw new Error(`Unexpected public repository file: ${file}`)
