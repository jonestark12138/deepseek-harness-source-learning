...
function validateSite(root){
  ...
  for(const f of files){
    ...
    if(!/^(index\.html|evidence\.html|site\.css|\.nojekyll|previews\/lesson-01\.html|assets\/fonts\/(workshop-hand\.woff2|OFL\.txt|provenance\.json)|assets\/lesson-01\/reader\/fig-0[1-6]-[a-z-]+\.png)$/.test(rel))throw new Error(`Not publishable: ${rel}`)
    if(f.endsWith('.png')){
  ...
  for(const required of ['index.html','previews/lesson-01.html','evidence.html','assets/fonts/OFL.txt'])if(!fs.existsSync(path.join(root,required)))throw new Error(`Missing ${required}`)
...
if(require.main===module){
  ...
  if(process.argv.includes('--repository')){
    ...
    for(const file of files)if(!/^(reader\/|README\.md$|\.gitattributes$|\.github\/workflows\/pages\.yml$|scripts\/check-reader\.cjs$|releases\/lesson-01-v1\.0\.1\.md$)/.test(file))throw new Error(`Unexpected public repository file: ${file}`)
